...
1. Confirm `main` is green: the
   [Unit Tests workflow](https://github.com/TommyE123/docker-autoheal/actions/workflows/tests.yml)
...
3. `.github/workflows/docker-publish.yml` builds and pushes the Docker image to both
   Docker Hub and GHCR automatically (see [Publishing](publishing.md)) and updates the
...
| Docker image build + push (Docker Hub + GHCR) on version tag | Yes (`docker-publish.yml`) |
