...
class ConfigManager:
    ...
    def migrate_legacy_health_checks(self, docker_client, monitoring_engine) -> None:
        """
        Migrate legacy health checks stored under ephemeral Docker container IDs
        to stable container identifiers.

        Before PR #38, custom health checks were keyed by Docker container ID,
        which changes on container recreation. PR #38 switches to stable IDs.
        This method migrates old entries to stable IDs when possible.
